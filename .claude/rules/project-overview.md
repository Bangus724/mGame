...
mGame is a Roblox **1vAll hidden-role** arena (Killer vs Sheriff + Civilians). Full pitch: `README.md`. Design constraints: `.claude/rules/game-design.md`.
...
Require Controllers with `require(ReplicatedStorage.Controllers.X)` / `require(ServerScriptService.Controllers.X)`. Gameplay uses `game:GetService`; Fusion UI may use `game.ReplicatedStorage.Packages…`. Tune match numbers in `GameProperties`.
